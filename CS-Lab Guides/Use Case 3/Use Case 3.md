...
1.  Navigate to **C:\LabFiles folder** in the **File Explorer**.
...
> **$env:FABRIC_WORKSPACE_NAME = "Fourth Coffee Commerce - 0910"**
>
> ![](./media/image16.png)
...
> will use this terminal again in later exercises to avoid setting up
...
10. Once the script has completed, navigate back to the Fabric Workspace
    ...
    notice that two new items have been created: a Fabric Data
    Warehouse **fc_commerce_wh** and a Fabric
...
1.  From the left navigation pane, select workspace icon as ‘**Fourth
    Coffee Commerce - 0910**’ and again, select the **workspace** name.
...
> **SELECT c.customerId,**
>
> **(SELECT VALUE COUNT(1) FROM r IN c.recommendations) AS
> recommendationSets,**
>
> **(SELECT VALUE COUNT(1) FROM r IN c.recommendations JOIN mi IN
> r.menuItems) AS totalRecommendedItems,**
>
> **(SELECT VALUE ROUND(AVG(r.score), 4) FROM r IN c.recommendations) AS
> avgRecScore,**
>
> **(SELECT VALUE MIN(r.expiresAt) FROM r IN c.recommendations) AS
> nextExpiryUtc**
>
> **FROM customers c**
>
...
>
> **IIF(c.loyaltyPoints \>= 500, "Silver", "Bronze")) AS
> membershipTier**
>
> **FROM customers c**
>
...
>
> **FROM \[fc_commerce_cosmos\].\[fc_commerce_cosmos\].\[customers\] AS
> c**
>
...
6.  On the next page,
    expand **Tables** \> **fc_commerce_cosmos** schema, then select the
...
>
> // project needed fields and make sure Items is an array (empty if
> missing)
>
> | project
>
...
>
> Items = iif(isnull(column_ifexists("items", dynamic(null))),
> dynamic(\[\]), column_ifexists("items", dynamic(null))),
>
...
>
> LoyaltyPointsEarned =
> toint(coalesce(column_ifexists("loyaltyPointsEarned", int(null)), 0)),
>
> LoyaltyPointsRedeemed =
> toint(coalesce(column_ifexists("loyaltyPointsRedeemed", int(null)),
> 0))
>
...
> | extend
>
> TotalQuantity = iif(isnotnull(TotalQuantity_t), TotalQuantity_t,
> TotalQuantity_i),
>
> TotalAmount = iif(isnotnull(TotalAmount_t), TotalAmount_t,
> TotalAmount_i),
>
...
> }
>
> .create-or-alter function with (folder="Silver")
...
>
> | extend LineTotal = todouble(coalesce(Items.totalPrice, Quantity \*
> UnitPrice)),
>
> PaymentMethod = tostring(column_ifexists("paymentMethod",""))
>
> | order by TransactionId asc, EventTimestamp asc, MenuItemId asc, Size
> asc
>
> | serialize
>
> | extend LineNumber = row_number(1, prev(TransactionId) !=
> TransactionId)
>
> | extend
>
...
> DateKey, TimeKey,
>
> // keep if you merge to Customer/Shop keys upstream; drop if not
> needed
>
> CustomerId, ShopId,
>
...
> **vw_Pos_LineItems_Sales()**
>
> **| summarize TotalSales = sum(LineTotal), TotalQuantity =
> sum(Quantity) by MenuItemId, MenuItemName**
>
...
1.  Navigate to **Fourth Coffee Commerce – 0910 workspace** icon and
...
2.  From the top menu ribbon, select **→|
...
6.  On the left side of the notebook, you'll see the Explorer pane,
    select **Add data items** \> **Existing data sources**.
...
- **lakehouse_guid**: Enter **59a3d763-a868-4242-b81e-46d37e52592c**.The
  GUID of your lakehouse. You can find this in the txt file
...
2.  Open a new SQL query in your data warehouse by selecting the **New
    SQL Query** button in the warehouse page.
...
>
> **SELECT TOP 10 \* FROM dbo.FactSalesLineItems order by CreatedAt
> desc;**
>
> ![A screenshot of a computer Description automatically
...
- **COSMOS_ENDPOINT**: The endpoint URL of your Cosmos DB account. You
  can find this in the Cosmos DB account settings under **Connection
  String**.
...
> **dotnet run --project
> src/app/FourthCoffee.Blazor/FourthCoffee.Blazor/FourthCoffee.Blazor.csproj**
>
> ![A screenshot of a computer Description automatically
